...
/**
 * A simple test harness to showcase a Classifier using pre-learned weights from
 * our Kaggle competition entry.
 * 
 * @author Michael Lavelle
 */
public class PretrainedKaggleCompetionClassifierDemo extends
		SupervisedNeuralNetworkDemoBase<LayeredSupervisedFeedForwardNeuralNetwork, LayeredFeedForwardNeuralNetworkContext> {

	private static final Logger LOGGER = LoggerFactory.getLogger(PretrainedKaggleCompetionClassifierDemo.class);
	...
	protected LayeredSupervisedFeedForwardNeuralNetwork createSupervisedNeuralNetwork(int featureCount) {
		...
		DirectedComponentFactory directedComponentFactory = new DefaultDirectedComponentFactoryImpl(matrixFactory,
				axonsFactory, activationFunctionFactory);
		...
		Matrix layer1Weights = matrixFactory.createMatrixFromRowsByRowsArray(6, 81,
				helper.deserialize(float[].class, "layer1Weights"));
		Matrix layer1Biases = matrixFactory.createMatrixFromRowsByRowsArray(6, 1,
				helper.deserialize(float[].class, "layer1Biases"));

		Matrix layer3Weights = matrixFactory.createMatrixFromRowsByRowsArray(400, 600,
				helper.deserialize(float[].class, "layer3Weights"));
		Matrix layer3Biases = matrixFactory.createMatrixFromRowsByRowsArray(400, 1,
				helper.deserialize(float[].class, "layer3Biases"));

		Matrix layer4Weights = matrixFactory.createMatrixFromRowsByRowsArray(100, 400,
				helper.deserialize(float[].class, "layer4Weights"));
		Matrix layer4Biases = matrixFactory.createMatrixFromRowsByRowsArray(100, 1,
				helper.deserialize(float[].class, "layer4Biases"));

		Matrix layer5Weights = matrixFactory.createMatrixFromRowsByRowsArray(10, 100,
				helper.deserialize(float[].class, "layer5Weights"));
		Matrix layer5Biases = matrixFactory.createMatrixFromRowsByRowsArray(10, 1,
		...
		FeedForwardLayer<?, ?> secondLayer = new MaxPoolingFeedForwardLayerImpl(directedComponentFactory, axonsFactory,
				differentiableActivationFunctionFactory, new Neurons3D(20, 20, 6, false),
				new Neurons3D(10, 10, 6, false), matrixFactory, true, false, 2);

		FeedForwardLayer<?, ?> thirdLayer = new FullyConnectedFeedForwardLayerImpl(directedComponentFactory,
				axonsFactory, new Neurons3D(10, 10, 6, true), new Neurons3D(5, 5, 16, false),
				new DefaultSigmoidActivationFunctionImpl(), matrixFactory, layer3Weights, layer3Biases, false);

		FeedForwardLayer<?, ?> forthLayer = new FullyConnectedFeedForwardLayerImpl(directedComponentFactory,
				axonsFactory, new Neurons(400, true), new Neurons(100, false),
				new DefaultSigmoidActivationFunctionImpl(), matrixFactory, layer4Weights, layer4Biases, false);

		FeedForwardLayer<?, ?> fifthLayer = new FullyConnectedFeedForwardLayerImpl(directedComponentFactory,
				axonsFactory, new Neurons(100, true), new Neurons(10, false),
				new DefaultSoftmaxActivationFunctionImpl(), matrixFactory, layer5Weights, layer5Biases, false);

		return new LayeredSupervisedFeedForwardNeuralNetworkImpl(directedComponentFactory,
				Arrays.asList(firstLayer, secondLayer, thirdLayer, forthLayer, fifthLayer));
	}
	...
	protected NeuronsActivation createTrainingDataNeuronActivations(MatrixFactory matrixFactory) {
		...
		float[][] trainingDataMatrix = toFloatArray(
				loader.loadDoubleMatrixFromCsv("train.csv", new KagglePixelFeaturesMatrixCsvDataExtractor(), 1, 1001));

		NeuronsActivation act = new NeuronsActivationImpl(new Neurons(trainingDataMatrix[0].length, false),
		...
		// return
		// act.createDownstreamActivation(act.getNewActivationsFormat(matrixFactory, new
		// Neurons3D(28, 28 ,1, true)));
	}
	...
	@Override
	protected void showcaseTrainedNeuralNetworkOnTrainingSet(LayeredSupervisedFeedForwardNeuralNetwork neuralNetwork,
		...
		LayeredFeedForwardNeuralNetworkContext accuracyContext = new LayeredFeedForwardNeuralNetworkContextImpl(
				matrixFactory, 0, null, false);
	...
	@Override
	protected void showcaseTrainedNeuralNetworkOnTestSet(LayeredSupervisedFeedForwardNeuralNetwork neuralNetwork,
			NeuronsActivation testDataInputActivations, NeuronsActivation testDataLabelActivations,
			MatrixFactory matrixFactory) throws Exception {

		// Temporarily prevent the test set activations being closed throughout this
		// demo. TODO. Implement auto-locking of first input
		...
		LayeredFeedForwardNeuralNetworkContext autoEncoderNeuronVisualisationContext = new LayeredFeedForwardNeuralNetworkContextImpl(
				matrixFactory, 0, null, false);

		DirectedLayerContext hiddenNeuronInspectionContext = autoEncoderNeuronVisualisationContext.getLayerContext(0);
		...
		for (int j = 0; j < 6; j++) {
			...
			float[] neuronActivationMaximisingFeatures = neuronActivationMaximisingActivation
					.getActivations(matrixFactory).getRowByRowArray();
		...
		for (int i = 0; i < 100; i++) {
			...
			NeuronsActivation orignalActivation = new NeuronsActivationImpl(new Neurons(activations.getRows(), false),
					activations, NeuronsActivationFeatureOrientation.ROWS_SPAN_FEATURE_SET);
			...
			LayeredFeedForwardNeuralNetworkContext classifyingContext = new LayeredFeedForwardNeuralNetworkContextImpl(
					matrixFactory, 0, null, false);
	...
	protected NeuronsActivation createTrainingLabelNeuronActivations(MatrixFactory matrixFactory) {
		...
		float[][] testDataMatrix = toFloatArray(
				loader.loadDoubleMatrixFromCsv("train.csv", new SingleDigitLabelsMatrixCsvDataExtractor(), 1, 1001));
	...
	protected NeuronsActivation createTestSetLabelNeuronActivations(MatrixFactory matrixFactory) {
		...
		// Load Mnist data into double[][] matrices
		float[][] testDataMatrix = toFloatArray(
				loader.loadDoubleMatrixFromCsv("train.csv", new SingleDigitLabelsMatrixCsvDataExtractor(), 1001, 2001));
	...
	protected LayeredFeedForwardNeuralNetworkContext createTrainingContext(
		...
		LayeredFeedForwardNeuralNetworkContext context = new LayeredFeedForwardNeuralNetworkContextImpl(matrixFactory,
				0, null, true);
		...
		context.getLayerContext(0).getSynapsesContext(0).getAxonsContext(0, 0)
				.withLeftHandInputDropoutKeepProbability(0.5f);
		return context;
	}
}
