 ...
 *
 */
public class AutoEncoderDemo extends UnsupervisedNeuralNetworkDemoBase<AutoEncoder, AutoEncoderContext> {
	...
	protected AutoEncoder createUnsupervisedNeuralNetwork(int featureCount) {
		...
		FeedForwardLayer<?, ?> decodingLayer = new FullyConnectedFeedForwardLayerImpl(directedComponentFactory,
				axonsFactory, new Neurons(200, true), new Neurons3D(28, 28, 1, false),
				new DefaultSigmoidActivationFunctionImpl(), matrixFactory, false);
	...
	protected NeuronsActivation createTrainingDataNeuronActivations(MatrixFactory matrixFactory) {
		...
		DoubleArrayMatrixLoader loader = new DoubleArrayMatrixLoader(AutoEncoderDemo.class.getClassLoader());
	...
	@Override
	protected void showcaseTrainedNeuralNetwork(AutoEncoder autoEncoder, NeuronsActivation testDataInputActivations,
		...
		AutoEncoderContext autoEncoderNeuronVisualisationContext = new AutoEncoderContextImpl(matrixFactory, 0, 0,
				false);
		...
		for (int i = 0; i < 100; i++) {
			...
			LOGGER.info("Encoded a single image from " + orignalActivation.getFeatureCount() + " pixels to "
					+ encodedFeatures.getFeatureCount() + " features");
			...
			NeuronsActivation reconstructedFeatures = autoEncoder.decode(encodedFeatures, decodingContext);
			...
			LOGGER.info("Decoded " + encodedFeatures.getFeatureCount() + " features into an image with "
					+ reconstructedFeatures.getFeatureCount() + " pixels");
